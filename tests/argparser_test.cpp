...
TEST(ArgParserTestSuite, EmptyTest) { //пустой тест
...
TEST(ArgParserTestSuite, StringTest) { //парсинг строкового аргуента param1, он корректно распознается и значение value1 извлекается getstringvalue
...
TEST(ArgParserTestSuite, ShortNameTest) { //короткое имя -p= вместо param1
    ArgParser parser("My Parser");
...
TEST(ArgParserTestSuite, DefaultTest) { //рповеряет что если арг не был передан в ком строке то возвращается аргумент по умолчанию 
    ArgParser parser("My Parser");
...
//5
TEST(ArgParserTestSuite, NoDefaultTest) { //если обязательный арг param1 не был передан то программа должна вернуть false
    ArgParser parser("My Parser");
    parser.AddStringArgument("param1");
...
TEST(ArgParserTestSuite, StoreValueTest) {//проверятся корректное хранение значения param1 в переменной value
    ArgParser parser("My Parser");
    std::string value;
...
TEST(ArgParserTestSuite, MultiStringTest) {//проверятся парсинг нескольких строковых аргументов - тут конкретно двух param1 и param2
    ArgParser parser("My Parser");
    std::string value;
    parser.AddStringArgument("param1").StoreValue(value);
...
TEST(ArgParserTestSuite, IntTest) {//проверяется парсинг целочисленного интового аргумента
    ArgParser parser("My Parser");
...
TEST(ArgParserTestSuite, MultiValueTest) {//проверятся парсинг нескольких интовых аргументов
    ArgParser parser("My Parser");
...
//10
TEST(ArgParserTestSuite, MinCountMultiValueTest) {//проверяет минимальное количество значений для мультизначного аргумента, т е если кол-во значений аргумента param1 меньше минимального необходимого то парсинг не выполняется
    ArgParser parser("My Parser");
    std::vector<int> int_values;
...
TEST(ArgParserTestSuite, FlagTest) { //проверяет корректность парсинга флага
    ArgParser parser("My Parser");
...
TEST(ArgParserTestSuite, FlagsTest) {//проверяет парсинг нескольких флагов - проверка что несколько флагов могут быть становлены и что дефолтные значения и storevalue для флагов работают правильно
    ArgParser parser("My Parser");
    bool flag3 ;
...
TEST(ArgParserTestSuite, PositionalArgTest) {//проверяет парсинг позиционных аргументов
    ArgParser parser("My Parser");
...
//14
TEST(ArgParserTestSuite, PositionalAndNormalArgTest) {//проверяет сочитание позиционных и непозиционных аргументов
    ArgParser parser("My Parser");
    std::vector<int> values;
...
//15
TEST(ArgParserTestSuite, RepeatedParsingTest) {//проверяет возможно ли повторно запарсить аргументы
    ArgParser parser("My Parser");
...
//16
TEST(ArgParserTestSuite, HelpTest) {//проверяет что флаг --хелп правильно вызывает помощь
    ArgParser parser("My Parser");
    parser.AddHelp('h', "help", "Some Description about program");
...
TEST(ArgParserTestSuite, HelpStringTest) {
    ...
    std::string expectedHelp =
        "Argument parser";
    ASSERT_EQ(parser.HelpDescription(), expectedHelp);
}